...
class GameScene: SKScene, ObservableObject, SKPhysicsContactDelegate {
    ...
    //Track the location of touches on screen and update the paddle
    ...
    func makeBricks() {
        let rows = [
            ...
            Row(color: .green, positionY: UIScreen.main.bounds.height / 1.205),
            Row(color: .green, positionY: UIScreen.main.bounds.height / 1.24),
            Row(color: .yellow, positionY: UIScreen.main.bounds.height / 1.28),
            Row(color: .yellow, positionY: UIScreen.main.bounds.height / 1.325),
        ]
    ...
    func makeRows(color: UIColor, positionY: CGFloat) {
        ...
        let totalBrickWidthSize = brickWidth * CGFloat(numberOfBricks)
        let xOffset = (frame.width - totalBrickWidthSize) / 2
    ...
    func makeFloor() {
        floor.name = "floor"
        floor.position = CGPoint(x: UIScreen.main.bounds.width / 2, y: 15)
